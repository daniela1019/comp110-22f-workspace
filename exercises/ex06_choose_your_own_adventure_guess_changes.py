...
def correct_guess_message(end_guess_count: int) -> str:
    ...
    if end_guess_count > 1 and end_guess_count <= 8:
        return "Impressive\U0001F44D  You're pretty good for a human."
    if end_guess_count > 8:
...
def main() -> None:
    ...
    greet()
    chosen_upper_bound: int = int(input(f"{player}, please give an upper bound for the range of numbers for me to choose from (max of 15): "))
    while chosen_upper_bound > 15:
        chosen_upper_bound = int(input("Can't you follow directions? Choose an upper bound again (max of 15): "))
    print("Wonderful. I will think of a number from 0 to your upper bound, inclusive, and you must guess it. Beware: My guess changes every time you are wrong\U0000274C")
    correct: bool = False
